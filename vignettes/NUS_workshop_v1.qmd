...
::: question
**Loading IMC data from a tiff file** @Lijia
:::
...
img_path  <- "../2025_NUS/data/raw_image_example/MB0002_1_345_fullstack.tiff"
mask_path <- "../2025_NUS/data/raw_image_example/MB0002_1_345_cellmask.tiff"
sample_id <- "../2025_NUS/data/raw_image_example/MB0002_1_345"
...
metal_order <- read.csv("../2025_NUS/data/raw_image_example/channel_to_metal_order.csv",
                        header = FALSE, stringsAsFactors = FALSE)
names(metal_order) <- "metal"
metal_order$Channel <- seq_len(nrow(metal_order))

metal_map <- read.csv("../2025_NUS/data/raw_image_example/metal_to_marker.csv",
...
# ssign channel names to the multi-channel image
...
# Load data
load("../2025_NUS/data/breastCancer.RData")
data_sce = IMC
...
```{r, warning=FALSE, error=FALSE, message=FALSE}
coexp_df <- readRDS("../2025_NUS/data/coexp_df.rds")
...
avg_expr <- aggregate(t(assay(fergusonSPE, "norm")), 
                      by = list(CellType = IMC$high_level_category), 
                      FUN = mean) %>%
  tibble::column_to_rownames(var = "CellType")

avg_expr <- avg_expr[, c("CD20", "CD3", "CD68", "CD45", "CD44",
...
fergusonSPE <- normalizeCells(data_sce,
                        markers = useMarkers,
                        transformation = NULL,
...
norm = plot_marker_densities(fergusonSPE, sample_id_col = "metabricId", markers_to_plot = "CD3", assay_name = "norm")$CD3 + theme(legend.position = "none") + ggtitle("")
log = plot_marker_densities(fergusonSPE, sample_id_col = "metabricId", markers_to_plot = c("CD3"), assay_name = "logcounts")$CD3 + theme(legend.position = "none") + ggtitle("")
ggarrange(log, norm, ncol=2, labels=c("Log Counts","Normalized Counts"))
```
...
scfeatures_result <- readRDS("../2025_NUS/data/scfeatures_result.RDS")
...
```{r, warning=FALSE, error=FALSE, message=FALSE}
classifyr_result_IMC <- readRDS("../2025_NUS/data/classifyr_result_IMC.rds")
```
...
```{r, warning=FALSE, error=FALSE, message=FALSE}
survForestCV <- readRDS("../2025_NUS/data/survForestCV.RDS")
```
